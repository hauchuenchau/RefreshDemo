package houzhongzhou.refreshdemo.base;
...
import houzhongzhou.refreshdemo.api.ApiService;
import rx.Observer;
...
public class BaseActivity<T> extends AppCompatActivity implements SwipeRefreshLayout.OnRefreshListener, BaseQuickAdapter.RequestLoadMoreListener {
    ...
    private ApiService<List<T>> mPagingService;
    ...
    protected void startGetData(RecyclerView recyclerView,SwipeRefreshLayout swipeRefreshLayout,BaseQuickAdapter quickAdapter, ApiService<List<T>> pagingService){
        ...
        mPagingService.getData(currentPage, PAGE_SIZE, new Observer<List<T>>() {
            ...
            public void onNext(List<T> list) {
                if ((list != null && list.isEmpty())) {
                    Toast.makeText(BaseActivity.this,"没有更多数据了",Toast.LENGTH_SHORT).show();
                    mQuickAdapter.addData(list);
